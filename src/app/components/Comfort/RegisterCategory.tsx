...
export default function RegisterCategory({ selectedCategories, setSelectedCategories }: ComfortCategoryProps) {
    ...
    return (
        <>
            <span
                className={` ${isTablet ? 'w-[95%]' : 'w-[80%]'} mx-auto flex font-medium text-3xl text-nowrap mt-6 font-[Tenada] text-yellow-6`}>
                카테고리 설정
            </span>
            ...
            {selectedCategories.length >= 3 && (
                <p className={`${isTablet ? 'w-[95%]' : 'w-[80%]'} mx-auto text-red-500 text-xl mt-2`}>카테고리는 최대 3개까지만 가능합니다.</p>
            )}
        </>
    );
}
